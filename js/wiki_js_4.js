	...
	$.ajax({
		...
		success: function(response){
			if(response.jsonResult) {
				...
				location.href = '/wiki.html?q=' + response.jsonResultMessage;
			} else {
...
$(document).ready(function () {
	...
	// 			html += `<li><a href="#" onclick="saveWordToCookie('${item.word}'); location.href='/ko/${encodeURIComponent(item.word)}'; return false;">${item.word}</a></li>`;
	// 		});
	// 		$("#wikiList").html(html);
	// 	},
	// 	error: function(err){
...
function loadHistory() {
	...
	if(history) {
		...
		words.forEach(word => {
			...
			link.href = '/ko/' + encodeURIComponent(word);
			link.onclick = function(event) {
				event.preventDefault();

				const originUrl = '/ko/' + encodeURIComponent(word);
